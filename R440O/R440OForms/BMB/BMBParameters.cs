...
namespace R440O.R440OForms.BMB
{
    ...
    public static class BMBParameters
    {
        ...
        private static bool _сигнал
        {
            get
            {
                bool ВыполненаПроверкаЧерезМодуляцию = БМАПодключенВерно() && N18_MParameters.ПереключательПРД == 3 &&
                               N18_MParameters.ПереключательПрдБма12 == 7 &&
                               // Добавить условие для пула ЧТ + скорость 1.2!!!
                               (N18_MParameters.Проверить_комутацию(ГнездаН18.Контроль_Прм_Тлф1, ГнездаН18.КоммутацияПрм_Канал1_БМА1));
                bool ВыполненоЗамыканиеНаСебяПоДКБМА1 = БМА1ПодключенВерно() && BMA_M_1Parameters.КнопкаШлейфДК == 3;

                bool НаН18ПовернутыТумбелры = N18_MParameters.ПереключательПРД == 2 &&
                    N18_MParameters.ПереключательПРМ1 == 4 && N18_MParameters.ПереключательПРМ2 == 4;

                bool ПолученСигналЧерезКанал1БМА1 = БМА1ПодключенВерно() && НаН18ПовернутыТумбелры &&
                    N18_MParameters.Проверить_комутацию(ГнездаН18.КоммутацияПрм_Канал1_Б11,ГнездаН18.КоммутацияПрм_Канал1_БМА1)
                      && (N18_MParameters.ПереключательПрдБма12 == 1 || N18_MParameters.ПереключательПрдБма12 == 4);

                bool ПолученСигналЧерезКанал2БМА1 = БМА1ПодключенВерно() && НаН18ПовернутыТумбелры &&
                    N18_MParameters.Проверить_комутацию(ГнездаН18.КоммутацияПрм_Канал2_Б11,ГнездаН18.КоммутацияПрм_Канал1_БМА1)
                      && (N18_MParameters.ПереключательПрдБма12 == 2 || N18_MParameters.ПереключательПрдБма12 == 5);


                bool ПолученСигналЧерезКанал3БМА1 = БМА1ПодключенВерно() && НаН18ПовернутыТумбелры &&
                    N18_MParameters.Проверить_комутацию(ГнездаН18.КоммутацияПрм_Канал3_Б11,ГнездаН18.КоммутацияПрм_Канал1_БМА1)
                      && (N18_MParameters.ПереключательПрдБма12 == 3 || N18_MParameters.ПереключательПрдБма12 == 6);

                // ДОПИШИ ДЛЯ БМА2 ЕСЛИ ЕГО СДЕЛАЕШЬ!
               

                return ВыполненаПроверкаЧерезМодуляцию || ВыполненоЗамыканиеНаСебяПоДКБМА1 ||
                    ПолученСигналЧерезКанал1БМА1 || ПолученСигналЧерезКанал2БМА1 || ПолученСигналЧерезКанал3БМА1;
                /*
        ...
        private static bool БМАПодключенВерно()
        {
            ...
            return БМА1ПодключенВерно() || БМА1ПодключенВерно();
        }
        ...
        {
            return BMA_M_2Parameters.Питание && ПереключательНаправление == 2;
        }
        /// <summary>
