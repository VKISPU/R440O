...
namespace R440O.R440OForms.BMB
{
    ...
    internal static class BMBParameters
    {
                ...
                return КнопкаПитание == Кнопка.Горит && КнопкаПередачаВызоваДк == Кнопка.Горит
                       && (ПереключательРаботаКонтроль == 2 ||
                           (КнопкаСлСвязь == Кнопка.Горит && БМАПодключенВерно() &&
                            (BMA_M_1Parameters.КнопкаШлейфДК && ПереключательНаправление == 1
                            || BMA_M_2Parameters.КнопкаШлейфДК && ПереключательНаправление == 2)));
            }
        }
        ...
        {
            return КнопкаПитание == Кнопка.Горит && БМАПодключенВерно() 
                && КнопкаПередачаВызоваДк == Кнопка.Отжата
        ...
        {
            return  N15Parameters.ЛампочкаБМА_2 && BMA_M_2Parameters.КнопкаПитаниеВКЛ && ПереключательНаправление == 2
        ...
        {
            if ((Команда[0] == 0 && Команда[1] == 0) ||
                Команда[0] == -1 || Команда[1] == -1) return;
            if (КнопкаПитание == Кнопка.Горит) передачаЦифр = true;
        ...
        public static string ПриемКоманды
        {
            get
            {
                if (ПереключательРаботаКонтроль == 1 && БМАПодключенВерно() 
                    && ((ПереключательНаправление == 1 && !BMA_M_1Parameters.КнопкаШлейфДК) ||
                    (ПереключательНаправление == 2 && !BMA_M_2Parameters.КнопкаШлейфДК))) return string.Empty;

                return (ЛампочкаДк)
                    ? string.Empty + "0"
                    : БМАПодключенВерно() ? ПереданнаяКоманда : string.Empty;
            }
        }
