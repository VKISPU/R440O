...
namespace R440O.R440OForms.BMB
{
    ...
    public static class BMBParameters
    {
     ...
     /*   /// <summary>
        ...
        /// </summary>
        private static bool ПодсветкаНабораНеправльная()
        {
        ...
        */
        ...
        private static bool БМАПодключенВерно()
        {
            return /* N15Parameters.ЛампочкаБМА_2 && BMA_M_2Parameters.КнопкаПитаниеВКЛ && ПереключательНаправление == 2
                || N15Parameters.ЛампочкаБМА_1 && BMA_M_1Parameters.КнопкаПитаниеВКЛ && ПереключательНаправление == 1;
                    */
                BMA_M_1Parameters.КнопкаПитаниеВКЛ && ПереключательНаправление == 1 ||
                BMA_M_2Parameters.КнопкаПитаниеВКЛ && ПереключательНаправление == 2;
        }
        ...
        public static string НаборКоманды
        {
            get
            {
                if (!ПодсветкаНабора() && _СтараяКоманда == ""
                    || (Команда[0] == -1 && Команда[1] == -1)) return string.Empty;
                var symbol1 = Команда[0] != -1 ? string.Empty + Команда[0] : "-";
                var symbol2 = Команда[1] != -1 ? string.Empty + Команда[1] : "-";
                _СтараяКоманда =  symbol1 + symbol2;
                return _СтараяКоманда;
            }
        }
        private static string _СтараяКоманда;
        ...
        public static void ПередатьКоманду()
        {
           ...
           // ПереданнаяКоманда = НаборКоманды;
            ...
            else
            {
                ...
                (ПереключательНаправление == 2 && BMA_M_2Parameters.КнопкаШлейфДК))
                    &&(!ЛампочкаДк))
                ПереданнаяКоманда = НаборКоманды;
        ...
        public static string ПриемКоманды
        {
            get
            {
              /*  if (ПереключательРаботаКонтроль == 1 && БМАПодключенВерно() 
                ...
                // Обработка того, что показываем в методе ПередатьКоманду.
